...
const registerUser = async (req, res) => {
    try{
        const validated = await registerValidation.validateAsync(req.body, { abortEarly: false, })
        if(!validated) {
...
const loginUser = async (req, res) => {
    try{
        ...
        if(!user){
            return res.status(404).json({ message: "Username/Email atau Password salah!"})
        }
    ...
    try{
        ...
        jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, decoded) => {
            if(err || user._id.toString() !== decoded.id) {
                return res.status(403).json({ message: "Verifikasi refresh token gagal!" });
            }
